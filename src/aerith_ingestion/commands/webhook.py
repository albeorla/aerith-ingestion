"""Command for running the unified webhook server."""
...
from aerith_ingestion.services.webhook import app
...
@click.group()
def webhook():
    """Manage unified webhook server."""
    pass


@webhook.group()
def gcal():
    """Manage Google Calendar webhooks."""
    pass
...
@click.option(
    "--calendar-id", default="primary", help="Calendar ID to watch for changes"
)
@pass_context
def serve(ctx, host: str, port: int, reload: bool, calendar_id: str):
    """Run the unified webhook server.
    
    This server handles webhooks for:
    1. Google Calendar notifications
    2. Todoist notifications
    """
    try:
        # Set up Google Calendar webhook channel if needed
        ...
        logger.info(f"Starting unified webhook server at {host}:{port}")
        logger.info("Endpoints:")
        logger.info("  - /webhook/google-calendar")
        logger.info("  - /webhook/todoist")
